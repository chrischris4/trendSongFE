...
import { MUSIC_GENRES, artwork } from '../../constants/config';
      ...
      <div style={{ maxWidth: 780, margin: '0 auto', padding: '40px 16px 80px' }}>
        ...
        <div style={{ marginBottom: 48 }}>
          ...
          <h1 style={{ color: '#fff', fontSize: 'clamp(22px, 4vw, 32px)', fontWeight: 900, lineHeight: 1.2, marginBottom: 10 }}>
            Tendances musicales :<br />semaine du {getWeekLabel()}
          </h1>
        ...
        {topSong && (
          <div style={sec}>
            <h2 style={h2}>🎵 Titre de la semaine</h2>
            <Link href={`/songs/${slugify(topSong.name, topSong.appleId)}`} style={{ display: 'flex', gap: 20, backgroundColor: '#141414', border: '1px solid #2A2A2A', borderRadius: 12, overflow: 'hidden', textDecoration: 'none', marginBottom: 16 }}>
              ...
              <div style={{ padding: '16px 16px 16px 0', flex: 1 }}>
                <div style={{ color: '#A78BFA', fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: 1, marginBottom: 6 }}>#1 mondial cette semaine</div>
        ...
        {topAlbum && (
          <div style={sec}>
            <h2 style={h2}>💿 Album de la semaine</h2>
            <Link href={`/albums/${slugify(topAlbum.name, topAlbum.appleId)}`} style={{ display: 'flex', gap: 20, backgroundColor: '#141414', border: '1px solid #2A2A2A', borderRadius: 12, overflow: 'hidden', textDecoration: 'none', marginBottom: 16 }}>
              ...
              <div style={{ padding: '16px 16px 16px 0', flex: 1 }}>
                <div style={{ color: '#EC4899', fontSize: 11, fontWeight: 700, textTransform: 'uppercase', letterSpacing: 1, marginBottom: 6 }}>#1 album mondial</div>
        ...
        {stats && stats.topGenres?.length > 0 && (
          <div style={sec}>
            ...
            <p style={body}>
              ...
              Le genre <span style={hl}>{topGenreDef?.label ?? topGenre?.name}</span> domine avec <span style={accent}>{topGenre?.pct}%</span> des titres classés
              {stats.topGenres[1] && <>, devant <span style={hl}>{MUSIC_GENRES.find(g => g.id === stats.topGenres[1].genreId)?.label ?? stats.topGenres[1].name}</span> ({stats.topGenres[1].pct}%)</>}.
            </p>
                ...
                return (
                  <div key={g.genreId} style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                    ...
                    <Link href={`/songs/genre/${def?.slug ?? ''}`} style={{ color: '#ddd', fontSize: 13, fontWeight: 600, textDecoration: 'none', flex: 1 }}>
                      {def ? `${def.emoji} ${def.label}` : g.name}
                    </Link>
        ...
        {topArtist && (
          <div style={sec}>
            <h2 style={h2}>🌍 L'artiste qui écrase les charts</h2>
            <p style={body}>
        ...
        {stats && stats.topSongs?.length > 0 && stats.topAlbums?.length > 0 && (
          <div style={sec}>
            <h2 style={h2}>📊 Top 5 mondial de la semaine</h2>
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(280px, 1fr))', gap: 20 }}>
              {[
                { title: 'Titres', items: stats.topSongs.slice(0, 5), base: 'songs' },
                { title: 'Albums', items: stats.topAlbums.slice(0, 5), base: 'albums' },
                ...
                <div key={title}>
                  ...
                  {items.map((item, i) => (
                    <Link key={item.appleId} href={`/${base}/${slugify(item.name, item.appleId)}`}
                      ...
                      <span style={{ color: '#EC4899', fontSize: 12, fontWeight: 700, whiteSpace: 'nowrap' }}>{item.countryCount} pays</span>
                    </Link>
                  ))}
                </div>
              ))}
        ...
        <div style={{ backgroundColor: '#141414', border: '1px solid #2A2A2A', borderRadius: 12, padding: '24px 28px', textAlign: 'center' }}>
          <p style={{ color: '#fff', fontWeight: 700, fontSize: 16, marginBottom: 8 }}>Explorez tous les charts</p>
          <p style={{ color: '#888', fontSize: 13, marginBottom: 20 }}>Filtrez par pays, genre et découvrez les détails de chaque titre avec son extrait audio.</p>
